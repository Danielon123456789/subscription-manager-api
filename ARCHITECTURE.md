...
### 👨‍👩‍👧 Groups
- Un usuario crea un grupo → se convierte en `owner` automáticamente
- `owner_id` vive en `groups` como referencia directa (practicidad en queries)
- `group_members` es la tabla pivote users ↔ groups con campo `role`
...
| `groups` | Tiene `owner_id` directo para queries frecuentes |
...
| 7 | `owner_id` en `groups` | Solo en `group_members` | Practicidad — evita JOINs en queries frecuentes |
